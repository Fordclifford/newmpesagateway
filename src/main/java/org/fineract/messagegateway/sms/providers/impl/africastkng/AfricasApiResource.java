...
@RestController
@RequestMapping("/africastkng")
public class AfricasApiResource {

	private static final Logger logger = LoggerFactory.getLogger(AfricasApiResource.class);
	
	private final SmsOutboundMessageRepository smsOutboundMessageRepository ;
	
	@Autowired
	public AfricasApiResource(final SmsOutboundMessageRepository smsOutboundMessageRepository) {
    ...
    public ResponseEntity<Void> updateDeliveryStatus(@PathVariable("messageId") final Long messageId, @RequestBody final SMSReportResponse payload) {
    	...
    	if(message != null) {
    		...
    		message.setDeliveryStatus(AfricasStatus.smsStatus(report.getStatus().getGroupId()).getValue());
